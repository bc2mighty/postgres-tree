...
export const transformDbTree = async(dbTreeResult: CategoryAttributes[]) => {
    ...
    const tree = new Tree(dbTreeResult[0].id, parentNodeKey, rootNodePath[rootNodePath.length - 1]);
    ...
    for(let i = 1; i < dbTreeResult.length;i++) {
        ...
        newKey = Number(`${parentNodeKey}${id}`)
        treeKeys[splitPath[splitPath.length - 1]] = newKey;
        
        tree.insert(id, Number(parentNodeKey), newKey, splitPath[splitPath.length - 1])
    }
